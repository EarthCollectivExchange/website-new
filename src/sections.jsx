...
import { STATUS, VALUES, PAGES } from './data.js';
import { StatusBadge } from './tree.jsx';
import { Reveal, SectionHead, SendGate, useRevealRoot } from './components.jsx';
        ...
        {items.map((it, i) => {
          ...
          return (
            <Reveal as={Tag} key={i} delay={i * 70} href={clickable ? `#/${it.page}` : undefined}
              ...
              {clickable ? <span className="mono" style={{ color: "var(--accent)", fontSize: "0.78rem", marginTop: "0.9rem" }}>Explore &rarr;</span> : null}
            </Reveal>
          );
        })}
      </div>
...
function ValuesSection({ heading }) {
  return (
    <section className="section-sm">
      {heading ? <SectionHead>{heading}</SectionHead> : null}
  ...
  return (
    <section className="section-sm">
      ...
      <Reveal as="p" className="mono dim" style={{ textAlign: "center", marginTop: "1.4rem", fontSize: "0.8rem", letterSpacing: "0.06em" }}>
        &darr; each layer inherits the protections beneath it
      </Reveal>
    </section>
  );
}
...
function LangTable() {
  ...
  return (
    <section className="section-sm">
      <SectionHead eyebrow="Discouraged &rarr; approved">The words we choose</SectionHead>
      <Reveal className="langt">
        ...
        {pairs.map(([a, b], i) => (
          <div className="lt-row" key={i}>
            ...
            <span className="lt-arrow">&rarr;</span>
...
function CapMatrix() {
  ...
  return (
    <section className="section-sm">
      ...
      <Reveal as="p" className="mono dim" style={{ marginTop: "1rem", fontSize: "0.8rem" }}>
        Trust levels: unknown &middot; known &middot; trusted — each unlocks more, never bypassing consent.
      </Reveal>
    </section>
  );
}
...
function Pillars() {
  ...
  return (
    <section className="section-sm">
      <SectionHead eyebrow="Thirteen pillars &middot; all active">What the source is made of</SectionHead>
      ...
      <Reveal className="card" style={{ maxWidth: "640px" }}>
        {sent ? (
          <div className="stack" style={{ gap: "0.6rem", padding: "1rem 0" }}>
            ...
            <button className="btn btn-ghost" style={{ alignSelf: "flex-start", marginTop: "0.6rem" }} onClick={() => setSent(false)}>&larr; Back</button>
          </div>
        ) : (
          <form onSubmit={(e) => { e.preventDefault(); setSent(true); }}>
            <div className="stack" style={{ gap: "1rem" }}>
                ...
                <div className="pillset">
                  {roles.map(r => (
                    <button type="button" key={r} className="pill" onClick={() => setRole(r)}
                      style={role === r ? { borderColor: "var(--accent)", color: "var(--paper)", background: "transparent" } : undefined}>{r}</button>
                  ))}
                </div>
              </div>
              <div>
              ...
              <button className="btn btn-primary" style={{ alignSelf: "flex-start" }}>Offer to help <span className="arrow">&rarr;</span></button>
            </div>
          </form>
        )}
      </Reveal>
...
export function Section({ s }) {
  switch (s.kind) {
    ...
    case "values": return <ValuesSection {...s} />;
  ...
  return (
    <div className="container">
      <header className="page-head">
        <Reveal className="breadcrumb"><a href="#/home">&larr; EarthOS</a></Reveal>
        ...
        <Reveal delay={180} className="row wrap" style={{ gap: "1rem", alignItems: "center", marginBottom: "1.4rem" }}>
          ...
          <span className="dim mono" style={{ fontSize: "0.78rem" }}>{STATUS[page.status].blurb}</span>
        </Reveal>
          ...
          <div className="next-grid">
            {page.next.map((n, i) => (
              <Reveal as="a" key={i} delay={i * 70} href={`#/${n.page}`} className="card hover link next-card">
                <span className="lab">{PAGES[n.page]?.eyebrow || "Page"}</span>
                <span className="row" style={{ justifyContent: "space-between", gap: "0.6rem" }}>
                  <strong className="h3">{n.label}</strong><span className="arrow">&rarr;</span>
                </span>
              </Reveal>
            ))}
          </div>
