...
import { NAV, VALUES } from './data.js';
import { StatusBadge } from './tree.jsx';

export function useRevealRoot() { /* no-op */ }

export function Reveal({ children, delay = 0, className = "", as = "div", style: styleProp, ...rest }) {
  ...
  useEffect(() => {
    ...
    const safety = setTimeout(show, 1700);
    return () => {
...
export function Nav() {
  ...
  useEffect(() => {
    ...
    const hashHandler = () => { close(); setDrawer(false); };
    const clickHandler = (e) => { if (!e.target.closest(".nav-item")) close(); };
      ...
      <div className={`drawer ${drawer ? "open" : ""}`}>
        <div className="container">
          ...
          <div style={{ marginTop: "2rem" }}>
            <a className="btn btn-primary" href="#/contact" onClick={() => setDrawer(false)}>Collaborate <span className="arrow">&rarr;</span></a>
          </div>
        </div>
      </div>
    </>
    ...
    <footer className="footer">
      ...
      <div className="container" style={{ paddingTop: "clamp(48px,7vh,80px)" }}>
        ...
        <div className="footer-bottom">
          <span>EarthOS — concept site &middot; {new Date().getFullYear()}</span>
...
export function SendGate() {
  ...
  useEffect(() => { setSent(false); }, [consent]);
  return (
    <div className="gate reveal">
      <div className="gate-head">
        <div>
          <div className="eyebrow" style={{ marginBottom: 4 }}>Try it &middot; Consent Gate</div>
      ...
      <div className="gate-body">
        ...
        <div className={`gate-msg ${sent ? "sent" : (!consent ? "blocked" : "")}`}>
          <div className="mono dim" style={{ fontSize: "0.74rem", marginBottom: 6 }}>
            FROM: unknown trust &middot; TO: you
          </div>
          {sent
        ...
        <div className="row wrap" style={{ gap: "0.7rem" }}>
          <button className="btn btn-primary" disabled={!consent} style={{ opacity: consent ? 1 : 0.45 }}
            ...
            Send through the gate <span className="arrow">&rarr;</span>
          </button>
...
export function Deco({ size = 320, top, left, right, bottom, variant = "slow", opacity = 0.5 }) {
...
export function SectionHead({ children, eyebrow }) {
  return (
