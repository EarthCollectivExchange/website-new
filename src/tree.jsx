...
export function StatusBadge({ status, pulse, small }) {
  const s = STATUS[status];
  if (!s) return null;
  return (
    <span className={`badge ${s.cls} ${pulse ? "pulse" : ""}`} style={small ? { fontSize: "0.62rem", padding: "0.32em 0.6em" } : undefined}>
...
function TreeNode({ node, depth, defaultOpen }) {
  ...
  if (!hasKids) {
    return (
      <a className="tleaf" href={node.page ? `#/${node.page}` : "#"}>
        <span className="node-dot" style={node.status ? { background: "currentColor" } : undefined}></span>
        <span>{node.name}</span>
...
export function EcosystemTree({ compact }) {
  const data = window.EARTHOS_TREE;
  return (
    <div className="tree">
      {compact
