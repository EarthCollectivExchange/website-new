# EarthOS Messaging — Integration Map
...
These systems are live and functioning in the running app:

| System | Module | Status |
|--------|--------|--------|
| Local messaging | `lib/messaging/` | Active |
| Human Modes (7 modes) | `lib/messaging/modes.ts` | Active |
| Interface Depth (simple/advanced/developer) | `lib/messaging/modes.ts` | Active |
| i18n (7 locales) | `lib/i18n/` | Active |
| Preferences (unified context) | `lib/messaging/preferencesContext.tsx` | Active |
...
`lib/messaging/preferencesContext.tsx` is the single source of truth for all runtime user preferences.

```
PreferencesProvider
│
├── humanMode          → lib/messaging/modes.ts (load/save via STORAGE_KEYS.humanMode)
├── interfaceDepth     → lib/messaging/modes.ts (load/save via STORAGE_KEYS.interfaceDepth)
├── appMode            → lib/messaging/modes.ts (load/save via STORAGE_KEYS.appMode)
...
No component or lib file should hardcode `'earthos.*'` key strings.
...
**Integration steps:**
1. Import `recordStatsEvent` from `lib/stats/lightAnalyzer.ts` into messaging action sites
2. Fire events at natural action boundaries:
   - `app_opened` → in `/app/messaging/page.tsx` on mount
   - `message_sent` → in `MessageComposer.tsx` on send
   - `conversation_created` → in `NewConversationDrawer.tsx` on create
   - `file_prepared` → in `FileTransferPanel.tsx` on confirm
   - `voice_recorded` → in `VoiceRecorderPanel.tsx` on complete
   - `language_changed` → in `SettingsTab.tsx` on locale change
   - `mode_changed` → in `SettingsTab.tsx` on humanMode or interfaceDepth change
...
2. Add QLPA scan to `MessageBubble.tsx` render pass — wraps rendered text, no persistence
...
Wrap all existing QA panels (`SyncQAPanel`, `ConsentQAPanel`, `RelayQAPanel`, `CryptoDevPanel`) in `DeveloperOnly`. Confirm they are invisible in Simple and Advanced view. Acceptance: Simple view shows zero developer diagnostic content.
...
```
appConstants.ts
  └── modes.ts (reads STORAGE_KEYS)
  └── statsStore.ts (reads STORAGE_KEYS)
  └── preferencesContext.tsx (reads STORAGE_KEYS + imports from modes.ts)

featureFlags.ts
  └── completeAnalyzer.ts (reads isFeatureEnabled)

statsTypes.ts → statsPrivacy.ts → statsStore.ts → lightAnalyzer.ts
                                              └── completeAnalyzer.ts

i18n/context.tsx (I18nProvider) — parallel to PreferencesProvider, not nested inside it
preferencesContext.tsx (PreferencesProvider) — wraps messaging UI
```
...
| Role | Files |
|------|-------|
| Single source of truth for preferences | `lib/messaging/preferencesContext.tsx` |
...
| Single source of truth for mode types | `lib/messaging/modes.ts` |
